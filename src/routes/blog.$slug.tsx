...
export const Route = createFileRoute("/blog/$slug")({
  ...
  head: ({ loaderData, params }) => {
    ...
    if (!loaderData) {
      return {
        meta: [{ title: "Post not found — Usman Jatoi" }, { name: "robots", content: "noindex" }],
        links: [{ rel: "canonical", href: url }],
      };
    }
    const rawTitle = loaderData.seo_title || loaderData.title || "Blog";
    const title = truncate(`${rawTitle} — Usman Jatoi`, 60);
    const desc = truncate(loaderData.seo_description || loaderData.excerpt || rawTitle, 158);
    const image = loaderData.image || `${SITE}${coverImageUrl(params.slug)}`;
            ...
            "@graph": [
              {
                ...
                articleSection: (loaderData.categories || []).map(
