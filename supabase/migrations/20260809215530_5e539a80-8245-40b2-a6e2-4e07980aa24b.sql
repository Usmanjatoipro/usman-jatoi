ALTER TABLE public.wp_posts ADD COLUMN IF NOT EXISTS content_len integer;

CREATE OR REPLACE FUNCTION public.wp_posts_set_content_len()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  NEW.content_len = length(coalesce(NEW.content, ''));
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS wp_posts_content_len ON public.wp_posts;
CREATE TRIGGER wp_posts_content_len
  BEFORE INSERT OR UPDATE ON public.wp_posts
  FOR EACH ROW EXECUTE FUNCTION public.wp_posts_set_content_len();

CREATE INDEX IF NOT EXISTS wp_posts_content_len_idx ON public.wp_posts (content_len) WHERE status = 'publish';
...
BEGIN
  ...
  CREATE TEMP TABLE _audit_base ON COMMIT DROP AS
    SELECT id, post_type, path, permalink, seo_title, seo_description,
           featured_media_id, meta, coalesce(content_len, 0) AS content_len
    FROM wp_posts
    WHERE status = 'publish' AND coalesce(path, '') <> '';

  WITH dup_title AS (
    SELECT lower(btrim(seo_title)) AS t, count(*) AS n
    FROM _audit_base WHERE coalesce(seo_title, '') <> ''
    GROUP BY 1 HAVING count(*) > 1
  ),
  dup_desc AS (
    SELECT lower(btrim(seo_description)) AS d, count(*) AS n
    FROM _audit_base WHERE coalesce(seo_description, '') <> ''
    GROUP BY 1 HAVING count(*) > 1
  ),
  issues AS (
    ...
    FROM _audit_base b WHERE coalesce(b.featured_media_id, 0) = 0
    UNION ALL
  ...
  capped AS (
    SELECT * FROM (
      SELECT i.*, row_number() OVER (PARTITION BY i.kind ORDER BY i.path) AS rn FROM issues i
    ) x WHERE rn <= 1000
  ),
  inserted AS (
